...
function handlerUpdatePrenda(dataPrenda, id_sucursal) {
  ...
  if (data != null) {
    ...
    if (dataPrenda.precio.length > 0) {
      if (data.precio != dataPrenda.precio) {
        // Actualiza y agrega solo cuando cambia el precio
        // se desactiva el precio actual
        const queryUpdate = `UPDATE Listas_Precios 
            SET is_active = FALSE
...
function handlerSaveNota(event, dataNota) {
  ...
  const FE_dia = _babel_runtime_corejs3_core_js_stable_parse_int__WEBPACK_IMPORTED_MODULE_1___default()(fecha_entrega.split('/')[0]);
  const FE_mes = _babel_runtime_corejs3_core_js_stable_parse_int__WEBPACK_IMPORTED_MODULE_1___default()(fecha_entrega.split('/')[1]);
  const FE_anio = _babel_runtime_corejs3_core_js_stable_parse_int__WEBPACK_IMPORTED_MODULE_1___default()(fecha_entrega.split('/')[2]);
  const FE_date = FE_anio + '-' + FE_mes + '-' + FE_dia;
  const FR_dia = _babel_runtime_corejs3_core_js_stable_parse_int__WEBPACK_IMPORTED_MODULE_1___default()(fecha_recepcion.split('/')[0]);
  const FR_mes = _babel_runtime_corejs3_core_js_stable_parse_int__WEBPACK_IMPORTED_MODULE_1___default()(fecha_recepcion.split('/')[1]);
  const FR_anio = _babel_runtime_corejs3_core_js_stable_parse_int__WEBPACK_IMPORTED_MODULE_1___default()(fecha_recepcion.split('/')[2]);
